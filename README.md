...
`Python` `FastAPI` `LangChain` `Vector DB` `RAG`

---

### ✈️ Autonomous Crew Mobility System *(FYP — in progress)*
> Kenya Airways crew ground transport optimization

Combines a **Capacitated DVRPTW Genetic Algorithm** with **Random Forest Regression** for journey time prediction, wrapped in an offline-first Android app using Event Sourcing and Supabase Realtime.

`Kotlin` `Android` `Python` `FastAPI` `PostGIS` `Supabase` `Genetic Algorithms` `ML`
